...
/// 3点から三角形を構築
/// </summary>
/// <param name="_p0">三角形の頂点01の座標</param>
/// <param name="_p1">三角形の頂点02の座標</param>
/// <param name="_p2">三角形の頂点03の座標</param>
/// <param name="tri">結果出力用</param>
void ComputeTriangle(const Vector3& _p0, const Vector3& _p1, const Vector3& _p2, Triangle* tri)
{
	// 座標をコピー
	tri->P0 = _p0;
	tri->P1 = _p1;
...
bool Point2Box(const DirectX::SimpleMath::Vector2& point, const DirectX::SimpleMath::Vector2 & boxPos, RECT rect)
{
	...
	if (point.x > boxPos.x + width / 2.0f || point.x < boxPos.x - width / 2.0f)
		return false;
	if (point.y > boxPos.y + height / 2.0f || point.y < boxPos.y - height / 2.0f)
		return false;
